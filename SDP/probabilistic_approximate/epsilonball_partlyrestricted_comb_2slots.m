...
function [C,Cs,p] = epsilonball_partlyrestricted_comb_2slots(J_A,J_B,d,order,epsilon)
...
% C, CS in I1 O1 I2 O2 F (here F = to co)
% S in I1 O1 I2 O2 F

C = sdpvar(((d*d)^2)*d*2,((d*d)^2)*d*2,'symmetric','real');
Cs = sdpvar(((d*d)^2)*d*2,((d*d)^2)*d*2,'symmetric','real');
...
if isequal(order,[1 2]) % AB
    for i=1:N
        F = F + [linkFAST(kron(J_A(:,:,i),J_B(:,:,i)),Cs,[(d*d)^2 d*2])==p*linkFAST(kron(J_A(:,:,i),J_B(:,:,i)),S,[(d*d)^2 d*2])];
    end
elseif isequal(order,[2 1]) % BA
    for i=1:N
        F = F + [linkFAST(kron(J_B(:,:,i),J_A(:,:,i)),Cs,[(d*d)^2 d*2])==p*linkFAST(kron(J_A(:,:,i),J_B(:,:,i)),S,[(d*d)^2 d*2])];
    end
elseif isequal(order,[1 1]) % AA
    for i=1:N
        F = F + [linkFAST(kron(J_A(:,:,i),J_A(:,:,i)),Cs,[(d*d)^2 d*2])==p*linkFAST(kron(J_A(:,:,i),J_A(:,:,i)),S,[(d*d)^2 d*2])];
    end
elseif isequal(order,[2 2]) % BB
    for i=1:N
        F = F + [linkFAST(kron(J_B(:,:,i),J_B(:,:,i)),Cs,[(d*d)^2 d*2])==p*linkFAST(kron(J_B(:,:,i),J_B(:,:,i)),S,[(d*d)^2 d*2])];
    end    
end

F = F + [Cs>=0,C-Cs>=0,trace(C)==d^2,C==ProjSeqSuperChannel(C,[1 d d d d d*2])];
